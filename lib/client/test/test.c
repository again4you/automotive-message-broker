...
static void test_get_AirDistributionCID()
{
	struct AirDistributionCIDType *p;
	int ret = amb_get_AirDistributionCID_with_zone(&p, None);
	if (ret != 0) {
		fprintf(stderr, "Fail to %s\n", __func__);
		return ;
	}
...
static void test_get_AirDistributionRightKnob()
{
	struct AirDistributionRightKnobType *p;
	int ret = amb_get_AirDistributionRightKnob_with_zone(&p, None);
	if (ret != 0) {
		fprintf(stderr, "Fail to %s\n", __func__);
		return ;
	}
	fprintf(stderr, " == AirDistributionRightKnob ==\n");
	fprintf(stderr, "    Zone: %d\n", p->Zone);
	fprintf(stderr, "    Value: %u\n", p->Value);
	fprintf(stderr, "    ValueSequence: %d\n", p->ValueSequence);
	fprintf(stderr, "    Time: %f\n", p->Time);
