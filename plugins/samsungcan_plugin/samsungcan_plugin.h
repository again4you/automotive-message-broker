...
class SamsungCANPlugin : public AmbPluginImpl, public CANObserver {

public:
    SamsungCANPlugin(AbstractRoutingEngine* re, const std::map<std::string, std::string>& config, AbstractSource& parent);
    virtual ~SamsungCANPlugin(); // has to be virtual because of unit tests
...
//
    ...
    */
    ...
    typedef std::tuple<SamsungCANPlugin*, CANBus*, int, const can_frame> TimerData;
